...
.defines(function() {
  EntityCharacter = ig.Entity.extend({
    ...
    sprint: function() {
      if(this.standing && this.vel.y == 0 && !this.crouching && ig.input.pressed('sprint')) {
        this.maxVel.x *= 2;
        this.sprinting = true;
      }
      else if(!this.crouching && this.vel.y == 0 && ig.input.released('sprint')) {
        this.maxVel.x /= 2;
    ...
    shoot: function() {
      if( !this.sprinting && ig.input.pressed('shoot') ) {
