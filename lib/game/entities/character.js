...
.defines(function() {
  EntityCharacter = ig.Entity.extend({
    ...
    turn: function() {
      if(!this.sprinting && (ig.input.mouse.x + ig.game.screen.x) > this.center().x){ // flips when mouse is at right side
        this.flip = false
      }
      else if(!this.sprinting && (ig.input.mouse.x + ig.game.screen.x) < this.center().x){// flips when mouse is at left side
        this.flip = true
      }
      else if(this.sprinting && this.vel.x < 0){ // flips to direction of sprint (left)
        this.flip = true
      }
      else if(this.sprinting && this.vel.x > 0){ // flips to direction of sprint (right)
        this.flip = false
      }
    },
    ...
    shoot: function() {
      if(!ig.input.state("sprint") && !this.sprinting && ig.input.pressed('shoot') ) { // 360 shoot command
        var mouseX = (ig.input.mouse.x + ig.game.screen.x); // Figures out the x coord of the mouse in the entire world
        var mouseY = (ig.input.mouse.y + ig.game.screen.y); // Figures out the y coord of the mouse in the entire world
