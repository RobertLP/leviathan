...
.defines(function() {
  EntityEnemy = ig.Entity.extend({
    ...
    update: function() {
      ...
      this.detectPlayer();
      this.movementGeneral();
      if(this.spottedLeft || this.spottedRight){
        this.movementAttack();
      }
      else{
        this.movementPassive();
      }
      this.parent();
    },
    ...
    movementPassive: function() {
      // near an edge? return!
      if( !((this.pos.x > this.lastPosX - this.minTurnDistance) && (this.pos.x < this.lastPosX + this.minTurnDistance))
        && !ig.game.collisionMap.getTile(this.pos.x + (this.flip ? 1 - this.size.x : this.size.x + 1),this.pos.y + this.size.y+1 ) ) {
    ...
    handleMovementTrace: function ( res ) {
      ...
      if (!((this.pos.x > this.lastPosX - this.minTurnDistance) && (this.pos.x < this.lastPosX + this.minTurnDistance)) && res.collision.x && !this.spottedLeft && !this.spottedRight){
        this.lastPosX = this.pos.x;
        this.flip = !this.flip;
      }
