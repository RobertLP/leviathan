...
EntityMainCharacter = EntityCharacter.extend({
  ...
  running: function() {
    if(this.standing /*&& this.timer.delta() > 0*/ && this.crouched == false && ig.input.state('left') && ig.input.state('sprint')){
      this.accel.x = -700
      this.maxVel.x = 400
    }
    if(this.standing /*&& this.timer.delta() > 0*/ && this.crouched == false && ig.input.state('right') && ig.input.state('sprint')){
      this.accel.x = 700
      this.maxVel.x = 350
    }
  },
