...
EntityMainCharacter = EntityCharacter.extend({
  ...
  init: function(x,y,settings) {
    ...
    this.sprinttimer = new ig.Timer(5);
    this.cooldowntimer = new ig.Timer(20)
  ...
  running: function() {
    if(this.standing && this.sprinttimer.delta() < 0 && this.crouched == false && ig.input.state('left') && ig.input.state('sprint')){
      this.accel.x = -700
      this.maxVel.x = 400
      this.sprinting = true
    }
    if(this.standing && this.sprinttimer.delta() < 0 && this.crouched == false && ig.input.state('right') && ig.input.state('sprint')){
      this.accel.x = 700
      this.maxVel.x = 400
      this.sprinting = true
    }
    ...
    ig.show( 'timerC', this.cooldowntimer.delta() );
  },
